...
describe('unified-e-invoice.handler', () => {
    ...
    it('registriert default Adapter und verarbeitet Batch erfolgreich', async () => {
        ...
        await jest.isolateModulesAsync(async () => {
            ...
            const event: SQSEvent = {
                Records: [<SQSEvent['Records'][number]>{messageId: 'm1'}, <SQSEvent['Records'][number]>{messageId: 'm2'}]
            }
    ...
    it('wirft bei nicht unterstütztem Adapter', async () => {
        ...
        await jest.isolateModulesAsync(async () => {
            await expect(import('../../../src/handlers/unifiedEInvoiceProcessor')).rejects.toThrow(
                "Adapter 'unknown.adapter' not yet implemented"
            )
        })
    })
})
