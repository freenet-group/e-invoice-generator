...
describe('dlqProcessor', () => {
    ...
    describe('SNS Alert', () => {
        ...
        it('sollte SNS Alert für mehrere Records senden', async () => {
            ...
            await handler(makeSqsEvent([makeRecord(), makeRecord({messageId: 'msg-002'})]), <never>{}, () => undefined)
    ...
    describe('SNS Fehler', () => {
        it('sollte bei SNS Fehler weiterlaufen und nächsten Record verarbeiten', async () => {
            snsMock.on(PublishCommand).rejectsOnce(new Error('SNS nicht erreichbar')).resolves({MessageId: 'sns-002'})
        ...
        it('sollte bei SNS Fehler keinen Fehler werfen', async () => {
            ...
            await expect(handler(makeSqsEvent([makeRecord()]), <never>{}, () => undefined)).resolves.toBeUndefined()
        })
    })
})
